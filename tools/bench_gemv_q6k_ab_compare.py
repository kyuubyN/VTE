"""
Same-process, interleaved A/B comparison of the original gemv_q6k_kernel
(git HEAD) against the header-coalescing fix (current working tree),
against the SAME read-bandwidth baseline -- same methodology as
tools/bench_gemv_q4k_ab_compare.py (Thesis V6), built after a naive
cross-process comparison there turned out to be inflated by GPU clock
variance between separate `python` process launches.

Usage:
    python tools/bench_gemv_q6k_ab_compare.py
"""
import ctypes
import os
import statistics
...
ORIGINAL_GEMV_PATH = os.path.join(
    ...
    "_scratch_gemv_q6k_original.hip"
)
...
def generate_q6_k_blocks(num_rows: int, row_bytes: int) -> bytes:
    ...
    for r in range(num_rows):
        for sb in range(n_sb):
            ...
            for i in range(128):
                blocks[offset + i] = np.random.randint(0, 256)
            for i in range(64):
                blocks[offset + 128 + i] = np.random.randint(0, 256)
            for i in range(16):
                blocks[offset + 192 + i] = np.random.randint(0, 256)
            blocks[offset + 208:offset + 210] = np.array([0.01], dtype=np.float16).tobytes()
    return bytes(blocks)
...
def main():
    ...
    if not os.path.exists(ORIGINAL_GEMV_PATH):
        ...
        print("Extract it first: git show HEAD:vte/compiler/templates/gemv_q6k.hip.template > _scratch_gemv_q6k_original.hip")
        sys.exit(1)
    ...
    out_block_orig = hip.safe_malloc(OUT_FEATURES * 2, "bench_out_orig")
    out_block_fix = hip.safe_malloc(OUT_FEATURES * 2, "bench_out_fix")
    ...
    fix_hsaco = codegen.compile_kernel("gemv_q6k", arch=arch, force_recompile=True)
    _, fix_fn = hip.load_kernel(fix_hsaco, "gemv_q6k_kernel")

    _, orig_fn = _compile_standalone(hip, scratch_dir, arch, ORIGINAL_GEMV_PATH, "gemv_q6k_original", "gemv_q6k_kernel")

    dst_scalar = hip.safe_malloc(OUT_FEATURES * 4, "bench_dst_scalar")
    _, baseline_fn = _compile_inline(hip, scratch_dir, arch, BW_BASELINE_SRC, "bw_baseline_q6k", "bw_baseline_kernel")
    ...
    def launch_fix(i):
        args = [
            x_block, w_blocks[i % NUM_COPIES], out_block_fix,
            ctypes.c_int(1), ctypes.c_int(1),
            ctypes.c_int(IN_FEATURES), ctypes.c_int(OUT_FEATURES),
            ctypes.c_void_p(0), ctypes.c_void_p(0), ctypes.c_float(1.0),
        ]
        hip.launch_kernel(fix_fn, grid=(OUT_FEATURES, 1, 1), block=(64, 1, 1),
                           args=args, shared_mem=0, expected_args=10)
    ...
    orig_gbps, fix_gbps, baseline_gbps = [], [], []
    for t in range(N_TRIALS):
        ...
        if t % 2 == 0:
            ...
            fix_ms = measure_one(hip, launch_fix, "ab_fix")
        else:
            fix_ms = measure_one(hip, launch_fix, "ab_fix")
            orig_ms = measure_one(hip, launch_orig, "ab_orig")

        orig_gb = (total_weight_bytes / 1e9) / (orig_ms / 1000.0)
        fix_gb = (total_weight_bytes / 1e9) / (fix_ms / 1000.0)

        orig_gbps.append(orig_gb)
        fix_gbps.append(fix_gb)
        baseline_gbps.append(baseline_gb)

        print(f"Trial {t+1}/{N_TRIALS} (order: {'orig->fix' if t % 2 == 0 else 'fix->orig'}): "
              f"baseline={baseline_gb:.1f} GB/s  original={orig_gb:.1f} GB/s ({orig_gb/baseline_gb*100:.1f}%)  "
              f"fix={fix_gb:.1f} GB/s ({fix_gb/baseline_gb*100:.1f}%)  "
              f"fix_vs_orig={(fix_gb/orig_gb - 1)*100:+.1f}%")
    ...
    print(f"fix      GB/s: median={statistics.median(fix_gbps):.1f}  min={min(fix_gbps):.1f}  max={max(fix_gbps):.1f}")

    med_orig_ratio = statistics.median(orig_gbps) / statistics.median(baseline_gbps)
    med_fix_ratio = statistics.median(fix_gbps) / statistics.median(baseline_gbps)
    print(f"\noriginal ratio to baseline (median/median): {med_orig_ratio*100:.1f}%")
    print(f"fix ratio to baseline (median/median):       {med_fix_ratio*100:.1f}%")

    per_trial_gain = [(fix_gbps[i] / orig_gbps[i] - 1) * 100 for i in range(N_TRIALS)]
    print(f"\nPer-trial fix-vs-orig gain: {[f'{g:+.1f}%' for g in per_trial_gain]}")
    ...
    print(f"trials favoring fix: {n_positive}/{N_TRIALS}")
