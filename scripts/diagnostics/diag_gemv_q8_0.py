...
from vte.compiler.dequantizer import dequantize_q8_0
...
def make_q8_0(n_blocks, seed):
    rng = np.random.default_rng(seed)
    blocks = np.zeros((n_blocks, 34), dtype=np.uint8)
    qs = rng.integers(-127, 128, size=(n_blocks, 32), dtype=np.int8)
    blocks[:, 2:34] = qs.view(np.uint8)
...
hsaco = eng.compile_kernel('gemv_q8_0', arch=arch)
mod, fn = hip.load_kernel(hsaco, 'gemv_q8_0_kernel')

def run_case(N, K, seed):
    n_blocks = N * (K // 32)
    raw = make_q8_0(n_blocks, seed)
    Wref = dequantize_q8_0(raw, N * K).reshape(N, K)
    ...
    hip.launch_kernel(function=fn, grid=(N,1,1), block=(64,1,1), args=args, shared_mem=0, expected_args=10)
    hip.synchronize()
    buf = bytearray(N * 2)
...
print("=== Validacao Q8_0 in-kernel vs dequantize_q8_0 ===")
d = [run_case(2560, 8192, 1), run_case(256, 2560, 2), run_case(512, 32, 3)]
worst = max(d)
